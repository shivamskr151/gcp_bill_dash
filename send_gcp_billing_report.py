...
SMTP_CONFIG = {
    ...
    "recipients": [
        "muskan.betla@gmail.com",  # Fixed typo: was @gamail.com
...
def fetch_billing_data_for_date(target_date: date) -> Tuple[float, Dict[str, float], str]:
    """
    ...
    return total_cost, service_costs, currency
...
def generate_billing_pdf(
    ...
    c.drawString(width - margin_x - 150, y, f"Cost ({currency})")
    y -= 15
    ...
    if service_costs:
        # Sort by cost (descending) to show highest costs first
        ...
        c.setFont("Helvetica", 11)
        c.drawString(margin_x, y, "No service-level data available for this date.")
        y -= 18
...
def main() -> None:
    ...
    try:
        total_cost, service_costs, currency = fetch_billing_data_for_date(target_date)
        print(f"✅ Fetched data: Total cost = {total_cost:,.2f} {currency}, Services = {len(service_costs)}")
    ...
    generate_billing_pdf(pdf_path, target_date, total_cost, service_costs, currency)
